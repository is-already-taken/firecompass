...
function(FBL, FBTrace) {
	// Get SourceLink reference to override some of its methods
	// SourceLink represents the link to a source in the CSS panel
	var SourceLinkRef = Firebug.getRep(new FBL.SourceLink());
	...
	SourceLinkRef.getTooltip = function(link) {
		var line = link.line,
			href = link.href,
			compassInfo,
			cache,
		...
		if (cache[href] && cache[href].compassInfoMap[line]) {
			// get compass info and prepare formatted string

			compassInfo = cache[href].compassInfoMap[line];
			compassInfoString += (compassInfo.isCompassInternal ? "[Compass] " : "") + FBL.$STRF("Line", [ compassInfo.path, compassInfo.line ]);
		}

		return decodeURI(FBL.$STRF("Line", [ link.href, link.line ]) + compassInfoString);
	};
